...
@section('body')
    <div class="container-fluid p-4">
        <div class="d-flex justify-content-between">
            <h3 class="mb-4">Sửa sản phẩm "{{ $sp->name }}"</h3>
            <div>
        ...
        <form class="row" action="{{ route('admin.update-product', ['id' => $sp->id]) }}" method="POST"
                                ...
                                <div class="input-group">
                                    <select class="form-select rounded-0" id="category" required>
                                        @foreach ($dsDanhMuc as $dm)
                                            <option
                                                value="{{ $dm->id }}"{{ $sp->category_id == $dm->id ? 'selected' : '' }}>
                                                {{ $dm->name }}</option>
                                        @endforeach
                                    </select>
            ...
            <div class="col-md-4 mb-4">
                ...
                <button type="submit" class="btn btn-primary btn-lg rounded-0 mt-4 w-100">Sửa Sản Phản</button>
            </div>
        </form>
    </div>
@endsection
